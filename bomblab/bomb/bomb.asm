...
0000000000400efc <phase_2>:
  400efc:	55                   	push   %rbp
  400efd:	53                   	push   %rbx
  400efe:	48 83 ec 28          	sub    $0x28,%rsp  # leave place for stack
  400f02:	48 89 e6             	mov    %rsp,%rsi   # rsi = rsp
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers>
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp) # compare rsp with 1
  400f0e:	74 20                	je     400f30 <phase_2+0x34>  # if equal, jump!
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>  # if not equal, boom!
  (Loop)
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>  # unconditional jump
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax        # eax = Memory[rbx-4], namely %rsp, so it's previous value is 1
  400f1a:	01 c0                	add    %eax,%eax              # eax = 2 * eax
  400f1c:	39 03                	cmp    %eax,(%rbx)            # (%rbx) compared with 8 
  400f1e:	74 05                	je     400f25 <phase_2+0x29>  # if equal, jump
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>  # if not equal, boom!
  400f25:	48 83 c3 04          	add    $0x4,%rbx              # rbx = rbx + 4, point to next element
  400f29:	48 39 eb             	cmp    %rbp,%rbx              # rbp compared with rbx
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>  # if not equal, jump
  (Loop_end)
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>  # jump
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx         # (%rbx) = (%rsp) + 4 = 27
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp        # (%rbp) = (%rsp) + 18 = 41
  400f3a:	eb db                	jmp    400f17 <phase_2+0x1b>  # jump
...
0000000000400f43 <phase_3>:
  400f43:	48 83 ec 18          	sub    $0x18,%rsp             # open stack
  400f47:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx         # rcx = M[%rsp+12]
  ...
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)         # compare M[%rsp+8], namely rdx, to 7
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>  # if rdx > 7, then jump to 0x400fad, bomb!
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax         # eax = rdx, let rdx = 1, then eax = 1
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8)     # jmp to memory 0x402478
  ...
  400fb9:	b8 37 01 00 00       	mov    $0x137,%eax            # 0x137
  400fbe:	3b 44 24 0c          	cmp    0xc(%rsp),%eax         # compare %eax with %rcx 
  400fc2:	74 05                	je     400fc9 <phase_3+0x86>  # if equal, jump to 400fc9
  400fc4:	e8 71 04 00 00       	callq  40143a <explode_bomb>
  400fc9:	48 83 c4 18          	add    $0x18,%rsp             # restore the stack
  400fcd:	c3                   	retq   

0000000000400fce <func4>:
  400fce:	48 83 ec 08          	sub    $0x8,%rsp  # restore stack
  400fd2:	89 d0                	mov    %edx,%eax  # eax = edx -> eax = 0xe
  400fd4:	29 f0                	sub    %esi,%eax  # eax -= esi -> eax = 0xe
  400fd6:	89 c1                	mov    %eax,%ecx  # ecx = eax -> ecx = 0xe
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx # ecx = 0x7
  400fdb:	01 c8                	add    %ecx,%eax  # eax += ecx -> eax = 0xe
  400fdd:	d1 f8                	sar    %eax       # eax = 0x7
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx # ecx = M[%rax + %rsi] -> M[15 + %rsi]
  400fe2:	39 f9                	cmp    %edi,%ecx  # edi = ecx = 0xe
...
000000000040100c <phase_4>:
  40100c:	48 83 ec 18          	sub    $0x18,%rsp      # open stack
  401010:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx  # rcx = M[%rsp+12]    
  401015:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx  # rdx = M[%rsp+8]
  40101a:	be cf 25 40 00       	mov    $0x4025cf,%esi  # %d %d
  40101f:	b8 00 00 00 00       	mov    $0x0,%eax       # eax = 0
  401024:	e8 c7 fb ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  401029:	83 f8 02             	cmp    $0x2,%eax       # eax = 2, or it will bomb!
  40102c:	75 07                	jne    401035 <phase_4+0x29>  # jump to bomb!
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp)  # compare 15 and rdx
  401033:	76 05                	jbe    40103a <phase_4+0x2e> # rdx = 0xe, or it will bomb!
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb>  # bomb!
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx       # edx = 0xe(wipe the high-order 32 bits)
  40103f:	be 00 00 00 00       	mov    $0x0,%esi       # esi = 0
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi  # edi = rdx -> edi = 0xe
  401048:	e8 81 ff ff ff       	callq  400fce <func4>  
  40104d:	85 c0                	test   %eax,%eax
  40104f:	75 07                	jne    401058 <phase_4+0x4c> # if equal, bomb!
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp)        # compare 0 and rcx
  401056:	74 05                	je     40105d <phase_4+0x51> # if equal, jump to 40105d
  401058:	e8 dd 03 00 00       	callq  40143a <explode_bomb>
  40105d:	48 83 c4 18          	add    $0x18,%rsp
  401061:	c3                   	retq   

0000000000401062 <phase_5>:
  401062:	53                   	push   %rbx             # save frame point
  401063:	48 83 ec 20          	sub    $0x20,%rsp       # open stack  
  401067:	48 89 fb             	mov    %rdi,%rbx        # rbx = rdi
  40106a:	64 48 8b 04 25 28 00 	mov    %fs:0x28,%rax    # canary value
  401071:	00 00 
  401073:	48 89 44 24 18       	mov    %rax,0x18(%rsp)  # M[%rsp+24] = %rax
  401078:	31 c0                	xor    %eax,%eax        # eax = 0
  40107a:	e8 9c 02 00 00       	callq  40131b <string_length>
  40107f:	83 f8 06             	cmp    $0x6,%eax        # compare if the length of string is 0x6
  401082:	74 4e                	je     4010d2 <phase_5+0x70> # if equal, jump to 4010d2
  401084:	e8 b1 03 00 00       	callq  40143a <explode_bomb> # else, bomb!
  401089:	eb 47                	jmp    4010d2 <phase_5+0x70>

  Loop:
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx    # %ecx = M[%rbx+%rax]
  40108f:	88 0c 24             	mov    %cl,(%rsp)            # M[%rsp] = %rcx
  401092:	48 8b 14 24          	mov    (%rsp),%rdx           # %rdx = M[%rsp] -> %rdx = %rcx
  401096:	83 e2 0f             	and    $0xf,%edx            
  401099:	0f b6 92 b0 24 40 00 	movzbl 0x4024b0(%rdx),%edx   # %edx = str1
  4010a0:	88 54 04 10          	mov    %dl,0x10(%rsp,%rax,1) # M[%rsp+%rax] = %edx
  4010a4:	48 83 c0 01          	add    $0x1,%rax             # rax += 1
  4010a8:	48 83 f8 06          	cmp    $0x6,%rax             # compare rax with 0x6
  4010ac:	75 dd                	jne    40108b <phase_5+0x29> # if rax != 0x6, jump to the loop 
  (Loop_end)
  
  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp)       # M[%rsp+16] = 0
  4010b3:	be 5e 24 40 00       	mov    $0x40245e,%esi        # esi = "flyers"
  4010b8:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi       # rdi = M[%rsp+10]
  4010bd:	e8 76 02 00 00       	callq  401338 <strings_not_equal>
  4010c2:	85 c0                	test   %eax,%eax
  4010c4:	74 13                	je     4010d9 <phase_5+0x77> # jump to 4010d9
  4010c6:	e8 6f 03 00 00       	callq  40143a <explode_bomb>
  4010cb:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)
  4010d0:	eb 07                	jmp    4010d9 <phase_5+0x77>
  4010d2:	b8 00 00 00 00       	mov    $0x0,%eax             # eax = 0
  4010d7:	eb b2                	jmp    40108b <phase_5+0x29> # jump to 40108b
  4010d9:	48 8b 44 24 18       	mov    0x18(%rsp),%rax       # rax = M[%rsp+24], restore the value
  4010de:	64 48 33 04 25 28 00 	xor    %fs:0x28,%rax
  4010e5:	00 00 
  4010e7:	74 05                	je     4010ee <phase_5+0x8c>
  4010e9:	e8 42 fa ff ff       	callq  400b30 <__stack_chk_fail@plt>
  4010ee:	48 83 c4 20          	add    $0x20,%rsp            # restore the stack 
  4010f2:	5b                   	pop    %rbx
  4010f3:	c3                   	retq   
...
00000000004010f4 <phase_6>:
  ...
  4010fa:	55                   	push   %rbp
  4010fb:	53                   	push   %rbx
  4010fc:	48 83 ec 50          	sub    $0x50,%rsp  # open stack
  401100:	49 89 e5             	mov    %rsp,%r13   # %r13 = %rsp
  401103:	48 89 e6             	mov    %rsp,%rsi   # %rsi = %rsp
  401106:	e8 51 03 00 00       	callq  40145c <read_six_numbers>  
  40110b:	49 89 e6             	mov    %rsp,%r14   # %r14 = %rsp
  40110e:	41 bc 00 00 00 00    	mov    $0x0,%r12d  # %r12d = 0
  (double loop, r12d and ebx act as index)
  (Loop1)
  # to check if the number in int r13[] is unique
  401114:	4c 89 ed             	mov    %r13,%rbp               # rbp = r13
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax          # eax = M[%r13]
  40111b:	83 e8 01             	sub    $0x1,%eax               # eax -= 1
  40111e:	83 f8 05             	cmp    $0x5,%eax               # compare eax and 5
  401121:	76 05                	jbe    401128 <phase_6+0x34>   
  401123:	e8 12 03 00 00       	callq  40143a <explode_bomb>
  401128:	41 83 c4 01          	add    $0x1,%r12d              # r12d += 1
  40112c:	41 83 fc 06          	cmp    $0x6,%r12d              # compare r12d and 6
  401130:	74 21                	je     401153 <phase_6+0x5f>   # if equal, jump
  401132:	44 89 e3             	mov    %r12d,%ebx              # ebx = r12d
  401135:	48 63 c3             	movslq %ebx,%rax               # rax = ebx -> rax = r12d
  401138:	8b 04 84             	mov    (%rsp,%rax,4),%eax      # eax = M[%rsp+4*%rax], rax is the index
  40113b:	39 45 00             	cmp    %eax,0x0(%rbp)          # compare eax with M[%rbp]
  40113e:	75 05                	jne    401145 <phase_6+0x51>   # if not equal, jump to 401145
  (Loop1_end)
  401140:	e8 f5 02 00 00       	callq  40143a <explode_bomb>   # if equal, bomb
  401145:	83 c3 01             	add    $0x1,%ebx               # ebx += 1
  401148:	83 fb 05             	cmp    $0x5,%ebx               # compare ebx and 5
  ...
  # for element in int r13[], x = 7 - x

  (Loop2)
  401160:	89 ca                	mov    %ecx,%edx               # edx = ecx -> edx = 7
  401162:	2b 10                	sub    (%rax),%edx             # edx -= M[%rax] -> edx = 7 - M[%rax]
  401164:	89 10                	mov    %edx,(%rax)             # M[%rax] = 7 - M[%rax]
  401166:	48 83 c0 04          	add    $0x4,%rax               # rax += 4, index += 1
  40116a:	48 39 f0             	cmp    %rsi,%rax               # compare rax and rsi
  40116d:	75 f1                	jne    401160 <phase_6+0x6c>   # if not equal, jump to 401160
  (Loop2_end) 
  ...
  (Loop3)
  401183:	ba d0 32 60 00       	mov    $0x6032d0,%edx         # edx = the address of the first node of the linked list
  401188:	48 89 54 74 20       	mov    %rdx,0x20(%rsp,%rsi,2) # M[%rsp+2*%rsi+32] = rdx
  40118d:	48 83 c6 04          	add    $0x4,%rsi              # rsi += 4
  401191:	48 83 fe 18          	cmp    $0x18,%rsi             # compare rsi and 24
  401195:	74 14                	je     4011ab <phase_6+0xb7>  # if rsi equals to 24, jump to 4011ab
  401197:	8b 0c 34             	mov    (%rsp,%rsi,1),%ecx     # ecx = M[%rsp+%rsi]
  40119a:	83 f9 01             	cmp    $0x1,%ecx              # compare ecx and 1
  40119d:	7e e4                	jle    401183 <phase_6+0x8f>  # if ecx <= 1, jump to 401183
  (Loop3_end)
  
  # rax act as the address of 
  # rsi act as the tail address of the linked list 
  # contain (80-40)/8 = 5 linked list

  # (%rax) is int r13[]
  # the index of linked list is r13[%rcx]
  # and the linked list is arranged as list[r13[0]], list[r13[1]], ...
  ...
  4011b0:	48 8d 44 24 28       	lea    0x28(%rsp),%rax        # address of rax = M[%rsp+40]
  4011b5:	48 8d 74 24 50       	lea    0x50(%rsp),%rsi        # address of rsi = M[%rsp+80]
  
  
  4011ba:	48 89 d9             	mov    %rbx,%rcx              # rcx = rbx
  4011bd:	48 8b 10             	mov    (%rax),%rdx            # rdx = M[%rax]
  4011c0:	48 89 51 08          	mov    %rdx,0x8(%rcx)         # M[%rcx+8] = rdx -> rdx = M[%rcx+8], point to next list
  4011c4:	48 83 c0 08          	add    $0x8,%rax              # rax += 8 
  4011c8:	48 39 f0             	cmp    %rsi,%rax              # compare rax and rsi
  4011cb:	74 05                	je     4011d2 <phase_6+0xde>  # if rax = rsi, jump to ...
  4011cd:	48 89 d1             	mov    %rdx,%rcx              # rcx = rdx, the loop begin to deal with next linked list
  ...
  # %rbx now stores M[%rsp+32], namely the start address of the linked list

  (Loop5)
  4011df:	48 8b 43 08          	mov    0x8(%rbx),%rax         # rax = M[%rbx+8]
  4011e3:	8b 00                	mov    (%rax),%eax            # rax = M[%rax] 
  
  # rax is the next list, rbx is the current list. 

  4011e5:	39 03                	cmp    %eax,(%rbx)            # compare M[%rbx] and eax
  4011e7:	7d 05                	jge    4011ee <phase_6+0xfa>  # if M[%rbx] >= eax, jump, ensure that the lists are in descending order
  4011e9:	e8 4c 02 00 00       	callq  40143a <explode_bomb>  # bomb! 
  4011ee:	48 8b 5b 08          	mov    0x8(%rbx),%rbx         # rbx = M[%rbx+8]  
  4011f2:	83 ed 01             	sub    $0x1,%ebp              # ebp -= 1
  4011f5:	75 e8                	jne    4011df <phase_6+0xeb>  # if ebp not equal 0, jump
  (Loop5_end)
  
  4011f7:	48 83 c4 50          	add    $0x50,%rsp             # restore the stack
  4011fb:	5b                   	pop    %rbx
  4011fc:	5d                   	pop    %rbp
...
0000000000401204 <fun7>:
  401204:	48 83 ec 08          	sub    $0x8,%rsp            # open stack
  401208:	48 85 ff             	test   %rdi,%rdi
  40120b:	74 2b                	je     401238 <fun7+0x34>
  40120d:	8b 17                	mov    (%rdi),%edx          # edx = M[%rdi]
  40120f:	39 f2                	cmp    %esi,%edx            # compare edx and rsi 
  401211:	7e 0d                	jle    401220 <fun7+0x1c>   # if edx <= rsi, jump
  401213:	48 8b 7f 08          	mov    0x8(%rdi),%rdi       # rdi = M[%rdi+8]
  401217:	e8 e8 ff ff ff       	callq  401204 <fun7>        # recursive call 
  40121c:	01 c0                	add    %eax,%eax            # eax += eax
  40121e:	eb 1d                	jmp    40123d <fun7+0x39>   
  401220:	b8 00 00 00 00       	mov    $0x0,%eax            # eax = 0
  401225:	39 f2                	cmp    %esi,%edx            # compare edx and rsi
  401227:	74 14                	je     40123d <fun7+0x39>   # if equal, jump to 40123d
  401229:	48 8b 7f 10          	mov    0x10(%rdi),%rdi      # rdi = M[%rdi+16]
  40122d:	e8 d2 ff ff ff       	callq  401204 <fun7>        # recursive call
  401232:	8d 44 00 01          	lea    0x1(%rax,%rax,1),%eax    # store eax in M[1+2*%rax]
  401236:	eb 05                	jmp    40123d <fun7+0x39>       # jump to 40123d
  401238:	b8 ff ff ff ff       	mov    $0xffffffff,%eax         # eax = 0xffffffff
  40123d:	48 83 c4 08          	add    $0x8,%rsp            # restore stack
  401241:	c3                   	retq   
...
0000000000401242 <secret_phase>:
  ...
  401252:	48 89 c7             	mov    %rax,%rdi                  # edi = rax
  401255:	e8 76 f9 ff ff       	callq  400bd0 <strtol@plt>        
  40125a:	48 89 c3             	mov    %rax,%rbx                  # rbx = rax
  40125d:	8d 40 ff             	lea    -0x1(%rax),%eax            # eax = M[%rax-1]
  401260:	3d e8 03 00 00       	cmp    $0x3e8,%eax                # eax equals to 1000  
  401265:	76 05                	jbe    40126c <secret_phase+0x2a>
  401267:	e8 ce 01 00 00       	callq  40143a <explode_bomb>      
  40126c:	89 de                	mov    %ebx,%esi                  # esi = ebx
  40126e:	bf f0 30 60 00       	mov    $0x6030f0,%edi             # edi = 36
  401273:	e8 8c ff ff ff       	callq  401204 <fun7>
  401278:	83 f8 02             	cmp    $0x2,%eax                  # compare eax and 2
  40127b:	74 05                	je     401282 <secret_phase+0x40> # if eax equals 2, jump -> eax should be 2
...
000000000040131b <string_length>:
  ...
  40131e:	74 12                	je     401332 <string_length+0x17> # if M[%rdi] equals to 0, jump
  401320:	48 89 fa             	mov    %rdi,%rdx                   # rdx = rdi
  401323:	48 83 c2 01          	add    $0x1,%rdx                   # rdx += 1
  401327:	89 d0                	mov    %edx,%eax                   # eax = rdx
  401329:	29 f8                	sub    %edi,%eax                   # eax -= edi
  40132b:	80 3a 00             	cmpb   $0x0,(%rdx)
  40132e:	75 f3                	jne    401323 <string_length+0x8>  # if not equal, jump to the front 
...
000000000040145c <read_six_numbers>:
  40145c:	48 83 ec 18          	sub    $0x18,%rsp      # rsi points to stack top
  401460:	48 89 f2             	mov    %rsi,%rdx       # rdx = rsi
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx  # rcx = rsi + 4
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax # rax = rsi + 14
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp)  # store rax onto stack
  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax # rax's location is rsi + 10
  ...
  40148a:	e8 61 f7 ff ff       	callq  400bf0 <__isoc99_sscanf@plt> # call sscanf
  40148f:	83 f8 05             	cmp    $0x5,%eax       # compare eax and 5
  401492:	7f 05                	jg     401499 <read_six_numbers+0x3d> # if eax > 5, jump
  401494:	e8 a1 ff ff ff       	callq  40143a <explode_bomb> # bomb!
  401499:	48 83 c4 18          	add    $0x18,%rsp      # restore stack 
  40149d:	c3                   	retq   
